...
test_256:
	bash /opt/local/bin/run_py_job.sh -e pytorch-CycleGAN-and-pix2pix -p cpu -c 4 -s test.py -- --dataroot datasets/corals2coral/testA --name c2c_cyclegan_256 --model test --no_dropout

test_416:
	bash /opt/local/bin/run_py_job.sh -e pytorch-CycleGAN-and-pix2pix -p cpu -c 4 -s test.py -- --dataroot datasets/corals2coral/testA --name c2c_cyclegan_416 --model test --no_dropout --load_size 416 --crop_size 416
